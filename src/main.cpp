...
//
//    Map map;
//    map.setFieldContainer(read_to_map("/MapLib/test_map.txt"));
//    Drawing drawUI;
...
//    }
    ...
    std::vector<std::vector<Field>> vector = read_to_map("../MapLib/test_map.txt");
    for(auto& vector1: vector){
        for(auto& field: vector1){
            std::cout<<field.isBarrier<<" ";
        }
        std::cout<<std::endl;
    }
    system("pause");
